...
namespace Ailurus
{
	...
	void RenderSystem::CalculateCascadeShadows()
	{
		...
		Vector3f forward = Vector3f(-cameraRotMat[2][0], -cameraRotMat[2][1], -cameraRotMat[2][2]).Normalize();
		Vector3f right = Vector3f(cameraRotMat[0][0], cameraRotMat[0][1], cameraRotMat[0][2]).Normalize();
		Vector3f up = Vector3f(cameraRotMat[1][0], cameraRotMat[1][1], cameraRotMat[1][2]).Normalize();
