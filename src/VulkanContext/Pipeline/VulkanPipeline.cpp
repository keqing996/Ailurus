...
namespace Ailurus
{
		...
		uint32_t pushConstantSize)
	{
		...
		// Push constant range (vertex stage for inverse VP matrix)
		vk::PushConstantRange pushConstantRange;
		pushConstantRange.setStageFlags(vk::ShaderStageFlagBits::eVertex)
			.setOffset(0)
			.setSize(pushConstantSize);
		...
		{
			Logger::LogError("Failed to create skybox pipeline layout: {}", e.what());
		}

		// Empty vertex input (fullscreen triangle uses gl_VertexIndex)
		vk::PipelineVertexInputStateCreateInfo vertexInputInfo;
		...
		// Rasterization: no cull for fullscreen triangle
		vk::PipelineRasterizationStateCreateInfo rasterizer;
		rasterizer.setDepthClampEnable(false)
			.setDepthBiasEnable(false)
			.setRasterizerDiscardEnable(false)
		...
		// Multisample: match current MSAA setting
		vk::PipelineMultisampleStateCreateInfo multisampling;
		multisampling.setSampleShadingEnable(false)
			.setAlphaToOneEnable(false)
			.setPSampleMask(nullptr)
		...
		// Depth stencil: depth test enabled (read-only), compare LessOrEqual for z=1.0 at far plane
		vk::PipelineDepthStencilStateCreateInfo depthStencil;
		depthStencil.setDepthTestEnable(true)
			.setDepthWriteEnable(false)
		...
		// Pipeline rendering create info (with depth attachment)
		vk::PipelineRenderingCreateInfo pipelineRenderingCreateInfo;
		pipelineRenderingCreateInfo.setColorAttachmentFormats(colorFormat)
			.setDepthAttachmentFormat(depthFormat)
			.setStencilAttachmentFormat(vk::Format::eUndefined);
		...
		try
		{
			...
			if (pipelineCreateResult.result == vk::Result::eSuccess)
				...
				Logger::LogError("Failed to create skybox graphics pipeline");
		}
		catch (const vk::SystemError& e)
		{
			Logger::LogError("Failed to create skybox graphics pipeline: {}", e.what());
		}
	}
} // namespace Ailurus
