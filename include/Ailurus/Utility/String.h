...
namespace Ailurus
{
    ...
    public:
        ...
        // Prevent dangling string_view from temporary strings
        static std::vector<std::string_view>
        SplitView(std::string&& inputStr, char delimiter) = delete;

        static std::vector<std::string_view>
        SplitView(std::string&& inputStr, const std::string& delimiter) = delete;
