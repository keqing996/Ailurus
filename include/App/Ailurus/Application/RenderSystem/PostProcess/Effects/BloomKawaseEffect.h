...
namespace Ailurus
{
    ...
    /// @brief Bloom post-process effect using the Kawase dual-filter blur variant.
    /// Uses dual Kawase filters for both downsample and upsample passes.
    /// Must be placed before ToneMappingEffect in the post-process chain (operates on HDR data).
    class BloomKawaseEffect : public PostProcessEffect
    {
    public:
        BloomKawaseEffect() = default;
        ~BloomKawaseEffect() override;
